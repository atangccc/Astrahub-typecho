...
async function reloadItems() {
  ...
  try {
    let response;
    if (isBrowseMode()) {
      response = await fetchNewsBrowse(PAGE_SIZE, "", onlyMyGalaxy.value);
    } else {
      const keyword = searchKeyword();
      response = keyword
        ? await fetchNewsSearch(keyword, PAGE_SIZE, "", onlyMyGalaxy.value)
        : await fetchNewsBrowse(PAGE_SIZE, "", onlyMyGalaxy.value);
    }
    items.value = Array.isArray(response.items) ? response.items : [];
    nextCursor.value = String(response.nextCursor || "").trim();
    hasMore.value = Boolean(response.hasMore);
    refreshedAt.value = response.generatedAt || "";
...
async function loadMoreItems() {
  ...
  try {
    let response;
    if (isBrowseMode()) {
      response = await fetchNewsBrowse(PAGE_SIZE, nextCursor.value, onlyMyGalaxy.value);
    } else {
      const keyword = searchKeyword();
      response = keyword
        ? await fetchNewsSearch(keyword, PAGE_SIZE, nextCursor.value, onlyMyGalaxy.value)
        : await fetchNewsBrowse(PAGE_SIZE, nextCursor.value, onlyMyGalaxy.value);
    }
    const more = Array.isArray(response.items) ? response.items : [];
    items.value = deduplicateAppend(items.value, more);
    nextCursor.value = String(response.nextCursor || "").trim();
    hasMore.value = Boolean(response.hasMore) && nextCursor.value !== "" && more.length > 0;
  } catch (e) {
...
async function reloadSources() {
  ...
  try {
    const response = await fetchNewsDiscover(SOURCE_LIMIT);
...
async function loadMoreSources() {
  ...
  try {
    const response = await fetchNewsDiscover(SOURCE_LIMIT, sourcesNextCursor.value);
    const more = Array.isArray(response.items) ? response.items : [];
